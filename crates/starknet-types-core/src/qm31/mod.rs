...
//! a, b, c and d represent a value from the Mersenne 31 field, denotated as M31. These four values can be interpreted
//! as the coordinates of a QM31.
...
///
/// The internal representation is composed of 4 coordinates, each representing a value
/// from the Mersenne 31 field.
/// Each of these coordinates can be represented by 31 bits.
#[repr(transparent)]
...
impl QM31 {
    ...
    pub fn add(&self, rhs: &QM31) -> QM31 {
        let coordinates1 = self.as_coordinates();
        let coordinates2 = rhs.as_coordinates();
    ...
    pub fn neg(&self) -> QM31 {
        let coordinates = self.as_coordinates();
        Self::reduce([
    ...
    pub fn mul(&self, rhs: &QM31) -> QM31 {
        let coordinates1_u64 = self.as_coordinates_u64();
        let coordinates2_u64 = rhs.as_coordinates_u64();
    ...
    pub fn inverse(&self) -> Result<QM31, QM31Error> {
        ...
        let coordinates = self.as_coordinates_u64();
...
mod test {
    ...
    fn test_qm31_packed_reduced_add() {
        ...
        let res = x.add(&y);
        let res_coordinates = res.as_coordinates();
        assert_eq!(
            res_coordinates,
            [
